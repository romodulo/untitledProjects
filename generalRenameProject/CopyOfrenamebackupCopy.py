...
# initial variables
sourceOfNames = "sourceOfNames.txt" 
dirOfVideosToRename = 'routerVueFirst4Videos'
directoryToHoldNewFilesProcessed = f"{dirOfVideosToRename}Modified"

#create a dirname
currentFile = "/home/romel-linux/untitledProjects/generalRenameProject/CopyOfrenamebackupCopy.py"
...
videosDir = 'Videos'
routerBackup = dirOfVideosToRename
...
modifiedList = os.path.join(dirName, sourceOfNames)
...
if len(newList) == len(processNL):
    emptyNewList = []
    eNL = emptyNewList
    #create a new path/to make life a little easier
    newlyCreatedPath = os.path.join(homeDir, videosDir, directoryToHoldNewFilesProcessed)
    #create a new dir
    if not os.path.exists(newlyCreatedPath):
        os.makedirs(newlyCreatedPath)
    else:
        pass
    ...
    for i in range(len(newList)):
        ...
        # # # crucial step:# # #
        shutil.copy(sourceDoc, targetDoc)
